...
function Details() {
	...
	const handleAchivementDelete = (id) => {
		const newList = achivementArray.filter(
			(singleAchivement) => singleAchivement.id !== id
		);
		setAchivementArray(newList);
	};

	const handleCompanyDelete = (id) => {
		const newList = companyArray.filter(
			(singleCompany) => singleCompany.id !== id
		);
	...
	const handleCompanyEdit = (id) => {
		...
		const filteredItems = companyArray.filter(
	...
	const handleAchivementEdit = (id) => {
		...
		const filteredItems = achivementArray.filter(
			(filterItem) => filterItem.id !== id
		);
